...
- **Use native `<a href>` for stream links**, not `EuiButtonEmpty` with `href` (which causes `about:blank#blocked`). Set `title` to the URL for hover visibility. Use `PlayStreamLink` (`src/player.jsx`): it keeps the native href but calls `onPlay(channel)` on a plain click. The parent renders a single `<PlayerSheet>` outside any table (see `ChannelsTab`, `TvPage`): table cells unmount when the layout switches between mobile cards and desktop columns (e.g. rotating a phone), which would close the player. External player deep links (`vlc-x-callback://`, `vlc://`, `intent://`) must also be plain anchors.
- **Keep focus inside the player sheet.** When a clicked button unmounts itself (e.g. "Resume in browser", the blocked-autoplay Play button), move focus into the player (the `<video>`), otherwise focus falls to `body` and Escape no longer closes the modal.
- **In-browser playback.** Live MPEG-TS plays through mpegts.js on a `<video playsinline muted>` with `disableRemotePlayback = true` (required for ManagedMediaSource on iPhone). Always destroy the player (`pause/unload/detachMediaElement/destroy`) when the sheet closes, the channel changes, or any mpegts `ERROR` fires (mpegts.js keeps downloading after media errors) — each open stream holds a provider connection. External player actions (VLC links, .m3u download, copy stream URL) must stop in-browser playback first so the external app gets a connection. A `play()` rejection with `NotAllowedError` is blocked autoplay: show a Play button, not a stall. Don't add request headers or `rangeLoadZeroStart`: the UI (UI port) fetches streams from the proxy port cross-origin, and the proxy's CORS preflight doesn't allow `Range`.
...
- **Keep the in-browser player buffered but bounded.** In `player.jsx` don't disable `enableStashBuffer` and don't chase low latency with the mpegts.js defaults (~0.5 s buffered → constant stalls), and don't disable `liveBufferLatencyChasing` entirely either (nor forget `liveBufferLatencyChasingOnPaused`, which is off by default and lets the buffer grow while paused): nothing else paces live loading, so a bursting provider fills the SourceBuffer, mpegts.js suspends the transmuxer and never resumes (ADR-016). Tune with a real-stream benchmark in Chrome (`channel: 'chrome'`, the bundled Chromium has no H.264/AAC) and measure the forward buffer over minutes, not seconds.
