...
All indices use the `metrics-*` naming convention which Elasticsearch recognises as metrics data streams.

## Data streams

| File | Data stream name | Type | Purpose |
|---|---|---|---|
| `metrics-iptv.sessions.json` | `metrics-iptv.sessions` | Standard data stream | Raw session lifecycle events (`session_start`, `session_end`, `session_error`) |
| `metrics-iptv.channel_metrics.json` | `metrics-iptv.channel_metrics` | **TSDB data stream** | Per-channel per-minute aggregates |
| `metrics-iptv.user_history.json` | `metrics-iptv.user_history` | Standard data stream | Completed sessions per user |

Replace `iptv` with your `--es-index-prefix` value (e.g. `metrics-myprefix.sessions`).
...
```
# 1. metrics-iptv.sessions.json       (PUT template + PUT _data_stream)
# 2. metrics-iptv.channel_metrics.json (PUT template + PUT _data_stream)
# 3. metrics-iptv.user_history.json   (PUT template + PUT _data_stream)
```

## Delete and recreate

```
DELETE _data_stream/metrics-iptv.sessions
DELETE _data_stream/metrics-iptv.channel_metrics
DELETE _data_stream/metrics-iptv.user_history

DELETE _index_template/metrics-iptv.sessions
DELETE _index_template/metrics-iptv.channel_metrics
DELETE _index_template/metrics-iptv.user_history
