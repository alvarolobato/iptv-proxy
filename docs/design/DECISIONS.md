...
- Worst case is bounded by the budget: `(1 + retries) × 2 × connect timeout` — ~32 s with defaults, in the same range as the single 30 s wait it replaces, but with a retry on a different stream server.
